...
const Dashboard = () => {
  ...
  const fetchEmergencyAlerts = async () => {
    try {
      // First fetch emergency alerts without the problematic join
      const { data: alerts, error: alertsError } = await supabaseAdmin
        .from('emergency_alerts')
        .select('id, status, notes, created_at, acknowledged_at, resolved_at, triggered_at, bus_id, conductor_id, lat, lng')
        .order('created_at', { ascending: false });
      ...
      // If we have alerts with bus_id, fetch bus details separately
      if (alerts && alerts.length > 0) {
        const busIds = alerts.map(a => a.bus_id).filter(Boolean);
        if (busIds.length > 0) {
          const { data: buses } = await supabaseAdmin
            .from('buses')
            .select('id, plate_number, route')
            .in('id', busIds);
          ...
          // Merge bus data into alerts
          const enrichedAlerts = alerts.map(alert => ({
            ...alert,
            buses: alert.bus_id ? busMap[alert.bus_id] : null
          }));

          setEmergencyAlerts(enrichedAlerts);
        } else {

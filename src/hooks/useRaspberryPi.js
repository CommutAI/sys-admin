...
export function useRaspberryPi(options = {}) {
  ...
  const connect = useCallback(async () => {
    try {
      ...
      const baseUrl = await getPiBaseUrl();
      if (!serverRunning) {
        throw new Error('Raspberry Pi server is not running at ' + baseUrl);
      }
      ...
      // Establish WebSocket connection (now async)
      wsRef.current = await createPiWebSocket(
  ...
  return {
    ...
    connectionStatus,
    error,
    raspberryPiUrl: null, // URL is now dynamic, use getPiBaseUrl() if needed

...
import { getRaspberryPiUrl } from './piAutoDiscovery';
...
// Dynamic URL that will be resolved via auto-discovery
let cachedRaspberryPiUrl = null;

/**
 * Get the current Raspberry Pi URL with auto-discovery
 */
async function getRaspberryPiBaseUrl() {
  if (cachedRaspberryPiUrl) {
    return cachedRaspberryPiUrl;
  }
  
  // Use auto-discovery to find the Raspberry Pi
  cachedRaspberryPiUrl = await getRaspberryPiUrl();
  return cachedRaspberryPiUrl;
}
...
export async function checkPiServer() {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const response = await fetchWithTimeout(`${baseUrl}/`, {}, DEFAULT_TIMEOUT);
...
export async function getPiHealth() {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const health = await fetchWithTimeout(`${baseUrl}/health`, {}, DEFAULT_TIMEOUT);
    return {
...
export async function getPiLocation() {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const location = await fetchWithTimeout(`${baseUrl}/location`, {}, DEFAULT_TIMEOUT);
    return {
...
export async function getPiHardwareStatus() {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const status = await fetchWithTimeout(`${baseUrl}/hardware-status`, {}, DEFAULT_TIMEOUT);
    return {
...
export async function getPiEmergencyStatus() {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const status = await fetchWithTimeout(`${baseUrl}/emergency-status`, {}, DEFAULT_TIMEOUT);
    return {
...
export async function getPiSmsLogs(limit = 100, offset = 0) {
  try {
    ...
    const logs = await fetchWithTimeout(
      `${baseUrl}/sms-logs?limit=${limit}&offset=${offset}`,
      {},
      DEFAULT_TIMEOUT
    );
    return logs.logs || [];
...
export async function getPiGpsLocations(tripId = null, limit = 100, offset = 0) {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const url = tripId 
      ? `${baseUrl}/gps-locations?trip_id=${tripId}&limit=${limit}&offset=${offset}`
      : `${baseUrl}/gps-locations?limit=${limit}&offset=${offset}`;
...
export async function getPiEmergencyAlerts(resolved = null, limit = 100, offset = 0) {
  try {
    ...
    const url = resolved !== null
      ? `${baseUrl}/emergency-alerts?resolved=${resolved}&limit=${limit}&offset=${offset}`
      : `${baseUrl}/emergency-alerts?limit=${limit}&offset=${offset}`;
...
export async function getPiSmsStatistics(days = 7) {
  try {
    ...
    const stats = await fetchWithTimeout(
      `${baseUrl}/sms-statistics?days=${days}`,
      {},
      DEFAULT_TIMEOUT
    );
    return stats;
...
export async function getPiGpsStatistics(days = 7) {
  try {
    const baseUrl = await getRaspberryPiBaseUrl();
    const stats = await fetchWithTimeout(
      `${baseUrl}/gps-statistics?days=${days}`,
      {},
      DEFAULT_TIMEOUT
    );
    return stats;
...
export async function getPiAddressLookup(lat, lon) {
  try {
    ...
    const address = await fetchWithTimeout(
      `${baseUrl}/address-lookup?lat=${lat}&lon=${lon}`,
      {},
      DEFAULT_TIMEOUT
    );
    return address;
 ...
 */
export async function getPiVideoFeedUrl() {
  const baseUrl = await getRaspberryPiBaseUrl();
 ...
 */
export async function getPiWebSocketUrl() {
  const baseUrl = await getRaspberryPiBaseUrl();
 ...
 */
export async function createPiWebSocket(onMessage, onOpen, onClose, onError) {
  const wsUrl = await getPiWebSocketUrl();
  let ws = null;
  let timeoutId = null;
 ...
 */
export async function getPiBaseUrl() {
  return await getRaspberryPiBaseUrl();
}

/**
 * Force rediscovery of Raspberry Pi (clears cache and rescans)
 */
export async function forceRediscovery() {
  cachedRaspberryPiUrl = null;
  return await getRaspberryPiBaseUrl();
}
